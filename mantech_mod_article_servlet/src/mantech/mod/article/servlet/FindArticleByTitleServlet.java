 ...
 * @author hoangvu
 */
@WebServlet(name = "FindArticleByTitleServlet", urlPatterns = "/findArticleByTitle")
public class FindArticleByTitleServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        List<Article> listArticle = articleBiz.findByLikeArticle(searchString);

        if (listArticle != null || listArticle.size()>0) {
            session.setAttribute("articleFindByTitle", listArticle);
        } else {
            session.setAttribute("articleFindByTitle", "fail");
        }
    } 
