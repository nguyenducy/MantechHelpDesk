...
public class ChangeArticleServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int d = Integer.parseInt(daySplit[1]);
        int m = Integer.parseInt(daySplit[2]);
        try {
            context = new InitialContext();
            ArticleBiz biz = (ArticleBiz) context.lookup("ejb/mantech/saigon/ArticleBiz");

            Article a = biz.findById(Integer.parseInt(id));
            a.setArticle(article);
            a.setContent(content);
            a.setCreatedDate(new Date(y-1900, m - 1, d));
            a.setRate(Integer.parseInt(rate));
            if (biz.editArticle(a)) {
