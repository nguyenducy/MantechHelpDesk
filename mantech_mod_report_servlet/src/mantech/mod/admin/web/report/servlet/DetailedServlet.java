 ...
 * @author NDC
 */
@WebServlet(name="DetailedServlet",urlPatterns="/detailedServlet")
public class DetailedServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
       resp.setContentType("application/pdf");
        ...
        TextColumnBuilder<Integer> ComplaintNo = col.column("ComplaintNo", "ComplaintNo", type.integerType());
       ...
       // TextColumnBuilder<String> TechnicianName = col.column("TechnicianName", "TechnicianName", type.stringType());
        ...
        TextColumnBuilder<Integer> EmployeeNo = col.column("EmployeeNo", "EmployeeNo", type.integerType());
        try {
            report() //.setColumnTitleStyle(null)
                    .setPageFormat(PageType.A4, PageOrientation.LANDSCAPE)
                    .setColumnTitleStyle(columnTitleStyle).highlightDetailEvenRows()
                    .columns(ComplaintNo, EmployeeName,  CreatedDate, CompletedDate,DepartmentName, TimeTaken, EmployeeNo)
                    .title(cmp.text("Detailed Report").setStyle(fontStyle))
                    .pageFooter(cmp.pageXofY().setStyle(boldCenteredStyle))
                    .pageFooter(cmp.text("Date:"))
                    .pageFooter(cmp.currentDate())
                    .setDataSource("Select * from R_Detailed  ", con.Getconnet())
                    .toPdf(out);
        } catch (DRException e) {
            e.printStackTrace();
        }
