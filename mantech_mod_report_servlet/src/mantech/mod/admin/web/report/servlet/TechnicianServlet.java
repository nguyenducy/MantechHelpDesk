      ...
      //  TextColumnBuilder<String> TechnicianName = col.column("TechnicianName", "TechnicianName", type.stringType());
        ...
        try {
            report() //.setColumnTitleStyle(null)
                    ...
                    .columns(CategoryName, EmployeeName, DepartmentName, CreatedDate, CompletedDate, TimeTaken)
                    .title(cmp.text("Technician Report").setStyle(fontStyle))
                    .pageFooter(cmp.pageXofY().setStyle(boldCenteredStyle))
                    .pageFooter(cmp.text("Date:"))
                    .pageFooter(cmp.currentDate())
                    .setDataSource("Select * from R_Technician order by CategoryName ", con.Getconnet())
                    .toPdf(out);
        } catch (DRException e) {
            e.printStackTrace();
        }
